        ...
        try:
            ...
            while True:
                ...
                if len(buffer) > 0:
                    ...
                    # If buffer is empty and epoch_end is set, we can sleep
                    # to avoid busy waiting. The parent will unset epoch_end
...
class Worker(CTX.Process):
    ...
    def prepare_dataset(self, **kwargs):
        ...
        if self.data.shuffle:
            ...
            self.data.stop = min(
                self.data.start + self.data.worker_size, len(self.data.dataset)
            )
    ...
    def collect_minibatch(self):
        ...
        if stop == len(self.data.indices):
            ...
            if self.data.shuffle:
                ...
                rotated_worker_index = (
                    self.worker_index + self.data.epoch + 1
                ) % self.data.nb_worker
