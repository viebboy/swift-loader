...
def verify_order(samples: list, dataset_size: int, batch_size: int, nb_workers: int):
    ...
    if set(samples) != set(expected_samples):
        ...
        raise AssertionError(
            f"Sample mismatch. Missing: {missing}, Extra: {extra}"
        )
    ...
    if nb_workers == 1:
        if samples != expected_samples:
            first_diff = next(
                (i for i, (a, b) in enumerate(zip(samples, expected_samples)) if a != b),
                None
            )
            raise AssertionError(
                ...
                for epoch in range(num_epochs):
                    ...
                    try:
                        ...
                        for consumer_idx, loader in enumerate(loaders):
                                ...
                                for batch in loader:
                                    ...
                                    if isinstance(batch, np.ndarray):
                                        ...
                                        if batch.ndim == 1:
                                            ...
                                            batch_list = list(range(len(consumer_samples), len(consumer_samples) + batch_size_actual))
                                    ...
                                    if (batch_list and 
                                        isinstance(batch_list[0], (list, tuple)) and 
                                        not isinstance(batch_list[0], np.ndarray) and
                                        not isinstance(batch_list[0], (int, float))):
                                        # Only flatten if the nested structure contains numbers, not arrays
                                        try:
                                            batch_list = [
                                                item for sublist in batch_list for item in sublist
                                            ]
                            ...
                            if nb_consumer > 1:
                                print(f"    Consumer {consumer_idx}: {consumer_batches} batches, {len(consumer_samples)} samples")
                    ...
                    verify_order(
                        ...
                        dataset_size,
                        batch_size,
                        worker_per_consumer * nb_consumer,  # Total workers across all consumers
                    )
...
def main():
    ...
    test_configs.append(
        {
            ...
            "dataset_class": IndexDataset,
            "dataset_kwargs": {"size": 97},  # Prime: 97/4 = 24.25 -> workers get 25, 24, 24, 24
            "batch_size": 4,
            "nb_consumer": 1,
    ...
    test_configs.append(
        {
            ...
            "dataset_class": IndexDataset,
            "dataset_kwargs": {"size": 53},  # Prime: 53/3 = 17.67 -> workers get 18, 18, 17
            "batch_size": 5,
            "nb_consumer": 1,
            "worker_per_consumer": 3,
            "shuffle": False,
    ...
    test_configs.append(
        {
            ...
            "dataset_class": IndexDataset,
            "dataset_kwargs": {"size": 101},  # Prime: 101/4 = 25.25 -> workers get 26, 25, 25, 25
            "batch_size": 7,
            "nb_consumer": 1,
            "worker_per_consumer": 4,
            "shuffle": False,
    ...
    test_configs.append(
        {
            ...
            "dataset_class": IndexDataset,
            "dataset_kwargs": {"size": 73},  # Prime: 73/5 = 14.6 -> workers get 15, 15, 15, 14, 14
            "batch_size": 6,
            "nb_consumer": 1,
    ...
    # ============================================================================
    # Test Case 4: Numpy arrays (order verification by checking indices) - uneven sizes
    # ============================================================================
    test_configs.append(
        {
            "name": "RandomNumpyDataset - 1 consumer, 2 workers, shuffle=False",
            "dataset_class": RandomNumpyDataset,
            "dataset_kwargs": {"size": 43, "shape": (20, 20)},  # 43/2 = 21.5 -> workers get 22, 21
            "batch_size": 5,
            "nb_consumer": 1,
            "worker_per_consumer": 2,
            "shuffle": False,
    ...
    test_configs.append(
        {
            "name": "RandomNumpyDataset - 1 consumer, 3 workers, shuffle=False",
            "dataset_class": RandomNumpyDataset,
            "dataset_kwargs": {"size": 47, "shape": (15, 15)},  # 47/3 = 15.67 -> workers get 16, 16, 15
            "batch_size": 4,
            "nb_consumer": 1,
            "worker_per_consumer": 3,
            "shuffle": False,
    ...
    # ============================================================================
    # Test Case 5: Multithread=False - uneven size
    # ============================================================================
    test_configs.append(
        {
    ...
    # ============================================================================
    # Test Case 6: Small batch size - uneven size
    # ============================================================================
    test_configs.append(
        {
    ...
    print(f"  All tests use shuffle=False")
    print()
