...
Usage:
    ...
    python tests/test_script_shuffle_true.py --multithread
"""
...
def test_configuration(
    ...
    **kwargs,
    ...
    try:
        ...
        # Calculate expected batches per consumer individually
        # Each consumer may have different number of batches, but should be
        # consistent across epochs
        expected_batches_per_consumer = [len(loader) for loader in loaders]
        expected_batches_total = sum(expected_batches_per_consumer)
        ...
        # Track batches per consumer per epoch to verify consistency
        consumer_batches_history = [[] for _ in range(nb_consumer)]
                        ...
                        for consumer_idx, loader in enumerate(loaders):
                            ...
                            try:
                                for batch in loader:
                                    ...
                                    # For IndexDataset, batch contains indices (1D array or list of integers)

                                    if isinstance(batch, np.ndarray):
                                        # 1D numpy array - indices from IndexDataset
                                    ...
                                    # Flatten if nested (IndexDataset returns simple integers, but handle nested structures)
                                    if (
                                        batch_list
                            ...
                            # Track batches for this consumer in this epoch
                            consumer_batches_history[consumer_idx].append(
                                consumer_batches
                            )

                            # Verify this consumer produced expected number of batches
                            expected = expected_batches_per_consumer[consumer_idx]
                            if consumer_batches != expected:
                                raise AssertionError(
                                    f"Consumer {consumer_idx} produced {consumer_batches} batches "
                                    f"in epoch {epoch+1}, expected {expected}. "
                                    f"Each consumer should produce consistent batches across epochs."
                                )

                            if nb_consumer > 1:
                                print(
                                    f"    Consumer {consumer_idx}: {consumer_batches} batches "
                                    f"(expected {expected}), {len(consumer_samples)} samples"
                                )
                            else:
                                print(
                                    f"    Consumer {consumer_idx}: {consumer_batches} batches, "
                                    f"{len(consumer_samples)} samples"
                                )
        ...
        # Verify batch consistency per consumer across epochs
        for consumer_idx in range(nb_consumer):
            batches = consumer_batches_history[consumer_idx]
            if len(set(batches)) != 1:
                raise AssertionError(
                    f"Consumer {consumer_idx} produced inconsistent batches across epochs: "
                    f"{batches}. Expected same number of batches in each epoch."
                )
        ...
        print(
            "  Batch consistency verified: each consumer produced "
            "consistent batches across epochs"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test SwiftLoader with shuffle=True configurations"
    )
    parser.add_argument(
        "--multithread",
        action="store_true",
        help="Enable multithread mode for all test configurations",
    )
    ...
    if args.multithread:
        print("\n⚠️  Multithread mode ENABLED (via --multithread flag)")
    print("=" * 80)
    ...
    for i, config in enumerate(test_configs, 1):
        ...
        try:
            ...
            # Override multithread if CLI argument is provided
            if args.multithread:
                config_copy["multithread"] = True
